...
def yaw_from_direction(src: Tuple[float, float], dst: Tuple[float, float]) -> float:
    """Calcula o yaw apontando de src para dst."""
...
class MissionOrchestratorNav2(Node):
    def __init__(self):
        ...
        # Nav2 action name (RELATIVO — o namespace do nó prefixa em runtime):
        self.declare_parameter("navigate_action", "navigate_to_pose")

        # Path topic do Nav2 (RELATIVO — vira robotN/plan dentro do namespace):
        self.declare_parameter("plan_topic", "plan")

        # Tópicos de UI (RELATIVOS — viram robotN/ui/...).
        # IMPORTANTE: a UI também precisa publicar/subscrever no namespace
        # certo (ex: /robot2/ui/start_mission) para isso funcionar.
        self.declare_parameter("ui_start_mission_topic", "ui/start_mission")
        self.declare_parameter("ui_mission_ctrl_topic", "mission_ctrl")
        self.declare_parameter("ui_progress_topic", "ui/progress")
        ...
        self.declare_parameter("missions_file", "")  # default vazio: launch deve passar
        ...
        # --- UI / controle ---
        ui_start_topic = self.get_parameter("ui_start_mission_topic").value
        ui_ctrl_topic = self.get_parameter("ui_mission_ctrl_topic").value
        ui_progress_topic = self.get_parameter("ui_progress_topic").value
        tts_topic = self.get_parameter("tts_topic").value

        self.create_subscription(String, ui_start_topic, self._on_ui_start, 10)
        self.create_subscription(String, ui_ctrl_topic, self._on_mission_ctrl, 10)

        self.pub_ui = self.create_publisher(String, ui_progress_topic, 10)
    ...
    def load_missions_from_json(self):
        file_path = self.get_parameter("missions_file").value
        if not file_path:
            self.get_logger().warn(
                "Parâmetro 'missions_file' vazio — nenhuma missão será carregada. "
                "Passe missions_file:=/path/to/missions.json no launch."
            )
            self.missions = {}
            return
        try:
            ...
            self.missions = {}
            for m_id, steps in raw_data.items():
                self.missions[m_id] = []
                for s in steps:
                    stype = s.get("type")
                    if stype == "Speak":
                        self.missions[m_id].append(Speak(text=s["text"]))
                    elif stype == "GoTo":
                        self.missions[m_id].append(GoTo(x=s["x"], y=s["y"], frame=s.get("frame", "map"), tag=s.get("tag")))
                    elif stype == "Wait":
                        self.missions[m_id].append(Wait(seconds=s["seconds"]))
                    elif stype == "Tour":
                        self.missions[m_id].append(Tour(waypoints=[tuple(x) for x in s["waypoints"]]))
                    elif stype == "ReturnHome":
                        self.missions[m_id].append(ReturnHome())
                    elif stype == "AwaitCommand":
                        self.missions[m_id].append(AwaitCommand(cmd=s["cmd"], label=s.get("label")))
            
            self.get_logger().info(f"✅ {len(self.missions)} missões carregadas de {file_path}")
        except Exception as e:
            self.get_logger().error(f"❌ Erro ao carregar missões: {str(e)}")
            self.missions = {}
    ...
    def _advance_tour(self, frame: str = "map"):
        ...
        # ← calcula yaw apontando para o próximo waypoint depois deste (se existir)
        #   assim o robô já chega orientado para onde vai continuar
        if self._tour_queue:
            nx, ny = self._tour_queue[0]
            auto_yaw = yaw_from_direction((x, y), (nx, ny))
        else:
            auto_yaw = None  # último waypoint: mantém orientação livre

        goal = pose_xy(x, y, frame, yaw=auto_yaw)
        self._blocked_until_nav_done = True
